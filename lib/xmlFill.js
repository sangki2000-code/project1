...
function moveTrailingTableRowsToEnd(xml, splitRowLabel) {
  ...
  function rebuildTableDef(rowCount, totalRows, newId, newZOrder) {
  ...
  const keptTableXml = rebuildTableDef(keptRows.length, totalRows, null, null) + keptRows.join("") + tblCloseTag;
  ...
  const movedTableXml =
    rebuildTableDef(movedRows.length, totalRows, newTblId, newZOrder) + renumberedMovedRows.join("") + tblCloseTag;
