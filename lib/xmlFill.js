...
/**
 * Same idea as wrapToWidth, but weights each character by how wide it
 * actually renders instead of treating every character as one unit. A plain
 * character count works only as long as the text is pure full-width Hangul —
 * once ASCII digits/punctuation/spaces (half-width, ~0.5 of a Hangul glyph)
 * are mixed in, a raw count either wraps too early (mostly-Hangul lines) or
 * lets a mostly-ASCII line run past the box (this was measured against real
 * Hangul rendering: a fixed character budget looked fine for one line and
 * overflowed on the next, purely because of how many ASCII characters each
 * line happened to contain).
 */
function visualWidth(text) {
  let width = 0;
  for (const ch of text) {
    width += ch.codePointAt(0) <= 0x7e ? 0.5 : 1;
  }
  return width;
}

function wrapToVisualWidth(text, maxWidth) {
  ...
  for (const word of words) {
    ...
    if (visualWidth(candidate) > maxWidth) {
      ...
      while (visualWidth(rest) > maxWidth) {
        // Walk forward one character at a time until adding the next one
        // would exceed the budget — cheap enough at these string lengths and
        // exact regardless of how the half/full-width characters are mixed.
        let cut = 0;
        let w = 0;
        for (const ch of rest) {
          const chWidth = ch.codePointAt(0) <= 0x7e ? 0.5 : 1;
          if (w + chWidth > maxWidth) break;
          w += chWidth;
          cut += ch.length;
        }
        cut = cut || 1;
...
function fillParagraphBlock(xml, label, nextLabel, value, options) {
  ...
  // 순수 한글 문장 기준 글자수(32~40)로 고정폭을 잡았더니, 숫자ㆍ영문ㆍ
  // 공백처럼 절반 폭인 문자가 섞인 줄은 너무 일찍 끊기고 순수 한글 줄은
  // 실제 칸을 넘치는 등 줄마다 결과가 달랐다(사용자가 한글에서 캡처해
  // 확인). 문자 폭을 반영해 계산하도록 바꿔 이 불일치를 없앤다 — 순수
  // 한글 기준 32자에 해당하는 폭.
  const lines = text
    .split(/\r\n|\r|\n/)
    .flatMap((line) => wrapToVisualWidth(line, 32));
