 ...
 *
 * `value` is split on real newlines only — each resulting paragraph is
 * written whole, with no manual line-wrapping, into one existing blank
 * paragraph in order. The paraPr here is JUSTIFY-aligned, so Hangul wraps
 * and justifies the paragraph's own text to the actual cell width when the
 * file is opened; guessing a wrap width ourselves (by character count, or by
 * a half/full-width-aware character count) only ever approximated that and
 * left visible slack or overflow depending on the mix of Hangul/ASCII in the
 * line. Leftover blank paragraphs (value shorter than the reserved space)
 * are left untouched. Leftover paragraphs (value longer than the reserved
 * space) become new ones, cloning the style of a sampled blank paragraph —
 * or of the label's own paragraph when the template reserved no blank lines
 * at all (e.g. the very last label in the section). Pass
 * `options.paraPrIDRef` to force a specific paragraph style instead (e.g.
 * the template's first-line-indent style for prose fields) rather than
 * cloning the sampled blank paragraph's.
 *
...
function fillParagraphBlock(xml, label, nextLabel, value, options) {
  ...
  const paraPrIDRef =
    (options && options.paraPrIDRef) || (sampledParaPrIDRef === "54" ? "56" : sampledParaPrIDRef);
  ...
  // 고정 글자수(32, 40)든 문자 폭 기반 계산이든, 우리가 직접 줄바꿈 위치를
  // 정하는 방식은 근본적으로 한글이 실제로 그리는 글자 폭을 추정에
  // 의존한다 — 사용자가 원하는 형태(문단 끝까지 양쪽 정렬로 꽉 채우고,
  // 실제 문단이 끝날 때만 줄을 바꾸는 방식)로는 만들 수 없다. 이 칸은
  // JUSTIFY 정렬이라 한글이 문단 내부 줄바꿈을 알아서 계산해 주므로,
  // 사용자가 입력한 줄바꿈(=문단 구분)마다 하나의 문단으로만 나누고
  // 문단 내부는 통째로 넘겨 한글이 실제 칸 너비에 맞춰 채우게 한다.
