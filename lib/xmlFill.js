...
function fillParagraphBlock(xml, label, nextLabel, value, options) {
  ...
  // 32자는 이전 글꼴(가로로 더 넓은 글꼴) 기준으로 잡았던 값이라, 휴먼명조로
  // 바꾼 뒤로는 실제 칸 너비를 다 채우지 못하고 오른쪽에 여백을 남긴 채
  // 일찍 줄바꿈되는 문제가 있었다(사용자 확인). 40자로 늘려 여백을 줄인다.
  const lines = text
    .split(/\r\n|\r|\n/)
    .flatMap((line) => wrapToWidth(line, 40));
