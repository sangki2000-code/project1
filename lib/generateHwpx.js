...
function fillSection0(xml, f) {
  ...
  // --- 표 셀 (라벨 옆 빈칸) ---
  // 피의자 성명/주민등록번호/직업/주거는 보안상 절대 채우지 않는다 (의도적으로 비움).
  ...
  // --- 별지(첨부) 페이지 ---
  // "범죄혐의를 인정할 만한 정황(증거관계)"은 별도 별지를 만들지 않고,
  // 범죄사실 별지 안에 소제목을 붙여 이어서 넣는다.
  const crimeFactsLines = [f.crimeFacts || ""];
  if (f.crimeCircumstances) {
    crimeFactsLines.push("", "범죄혐의를 인정할 만한 정황", f.crimeCircumstances);
  }

  out = appendAttachmentPages(out, [
    { heading: "압수할 물건", content: f.seizureItems || "" },
    { heading: "수색ㆍ검증할 장소, 신체 또는 물건", content: f.searchPlace || "" },
    { heading: "범죄사실 및 압수ㆍ수색ㆍ검증을 필요로 하는 사유", content: crimeFactsLines.join("\n") },
  ]);
