...
// 별지 서식에서 "(작성 안내) 피의자의 직업, ..." 안내 문구가 쓰는 문단서식 —
// 첫 줄 들여쓰기가 있고 양쪽 정렬이라, 줄글로 쓰는 칸(범죄사실/정황/필요성)에
// 그대로 재사용하면 한글이 문단 끝까지 알아서 채우고 실제 문단이 끝나는
// 곳에서만 줄을 바꾼다.
const PROSE_PARA_PR_ID = "12";
...
/**
 * 증거관계 선택 UI는 클릭할 때마다 한 줄씩 추가해 화면에서는 읽기 좋지만,
 * 문서에는 항목마다 줄바꿈되어 들어가면 오히려 빈 줄처럼 보여 공간을 차지한다.
 * 항목을 ", "로 이어붙여 한 문단으로 흐르게 하고, 줄바꿈은 문단 폭에 맞춰
 * fillParagraphBlock/wrapToWidth가 자동으로 처리하게 한다.
 */
function joinToSingleLine(value) {
  const items = String(value == null ? "" : value)
    .split(/\r\n|\r|\n/)
    .map((line) => line.trim())
...
function fillAttachmentSection0(xml, f) {
  ...
  // 범죄사실/정황/필요성처럼 줄글로 쓰는 칸은, 서식이 이미 첫 줄 들여쓰기가
  // 있는 문단서식(12번)을 안내 문구("작성 안내...")에 쓰고 있었다 — 그
  // 문단서식을 그대로 써서 사용자가 입력한 글이 그 안내 문구와 똑같이
  // 양쪽 정렬로 칸 끝까지 차고, 문단이 실제로 끝나는 곳에서만 줄이
  // 바뀌게 한다(번호 매긴 목록인 압수물건/장소는 항목마다 이미 줄을
  // 나눈 것이라 들여쓰기를 주지 않는다).
  out = fillParagraphBlock(out, "○ 범죄사실", null, f.crimeFacts || "", {
    boundaryTag: "</hp:subList>",
    paraPrIDRef: PROSE_PARA_PR_ID,
  });
  // 별지2: 압수수색검증을 필요로 하는 사유 (정황 / 필요성 2단 구성) — 두 라벨이 같은 셀 안에 있다.
  out = fillParagraphBlock(out, " - 범죄혐의의 정황", " - 압수수색의 필요성", joinToSingleLine(f.crimeContext), {
    paraPrIDRef: PROSE_PARA_PR_ID,
  });
  out = fillParagraphBlock(out, " - 압수수색의 필요성", null, f.necessityReason || "", {
    boundaryTag: "</hp:subList>",
    paraPrIDRef: PROSE_PARA_PR_ID,
  });
